...
function setupQuestion() {
  if (questionIndex < questions.getRowCount()) {
    ...
    radio = createRadio();
    radio.style('font-size', '24px'); // 放大選項字體
    options.forEach(option => radio.option(option));
    radio.position(width / 2 - 100, height / 2);
    
    submitButton = createButton('Submit');
    submitButton.style('font-size', '24px'); // 放大按鈕字體
...
function draw() {
  ...
  // 顯示左上角的文字
  textAlign(LEFT);
  textSize(24); // 放大左上角的文字
  text("413730481 張瑋玲", 10, 30);
}
